...
function fallbackRecommendation(data: EligibilityInput): EligibilityRecommendation {
...
export function buildEligibilityRecommendation(
  ...
  const matching = activeRules.filter((rule) => {
    if (
      ...
      rule.country &&
      rule.country.toLowerCase() !== data.preferred_country.toLowerCase()
    ) {
      return false;
    }
    if (
      ...
      !data.education_level.toLowerCase().includes(rule.education_level.toLowerCase()) &&
      !rule.education_level.toLowerCase().includes(data.education_level.toLowerCase())
    ) {
      return false;
    }
  ...
  const ruleCountries = [
    ...new Set(
      matching.map((r) => r.country).filter((c): c is string => Boolean(c))
    ),
  ];
