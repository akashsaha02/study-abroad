...
import { App, Button, Input, Result, Steps } from "antd";
...
export function EligibilityCheckerForm({
  ...
  const currentStep =
    preferredCountryId || preferredSubject ? 2 : name && phone ? 1 : 0;
  ...
  return (
    <PageLayout>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <PanelCard title={t("profile")} variant="glass">
          <Steps
            size="small"
            className="mb-6"
            current={currentStep}
        ...
        {result ? (
          <GlassPanelCard title={t("results")} variant="glass">
            <div className="stagger-children space-y-4 text-sm">
              <ResultRow
          ...
          >
            <Result
              ...
              className="max-w-sm [&_.ant-result-icon]:mb-2"
            />
          </SurfaceCard>
        )}
      </div>
