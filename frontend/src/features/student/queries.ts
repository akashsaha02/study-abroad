...
import { tableExists } from "@abroadly/shared/countries/fk-guard";
...
export async function getStudentByProfileId(profileId: string) {
  ...
  const countryMap = await resolveStudentCountryNames(supabase, [data]);
  const preferredCountryDisplay =
    countryMap.get(data.id) ?? data.preferred_country ?? null;

  return {
    ...data,
    preferred_country_ids: preferredCountryIds,
    preferred_country_display: preferredCountryDisplay,
  };
}
