...
export async function POST(
  ...
  if (existingStudent) {
    ...
    const { error: studentError } = await supabase.from("students").insert({
      profile_id,
      lead_id: leadId,
      preferred_country: studentFields.preferred_country,
      preferred_country_id: studentFields.preferred_country_id,
      preferred_subject: studentFields.preferred_subject,
      highest_education: studentFields.highest_education,
      cgpa: studentFields.cgpa,
      english_test_score: studentFields.english_test_score,
      budget: studentFields.budget,
      assigned_counselor_id: studentFields.assigned_counselor_id,
    });
